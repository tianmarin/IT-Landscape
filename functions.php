...
*/
function ita_security($capability="read_posts"){
	if ( !is_super_admin() ) :
...
function ita_class_menu_main(
	...
	<table class="wp-list-table widefat fixed posts">
			...
			<tr>
				...
				if($filters!=null){
					...
					foreach ($filters as $key => $filter):
						...
						endswitch;
					endforeach;
					wp_register_script('ita_formfilter',plugins_url( 'js/formfilter.js' , __FILE__),array( 'jquery' ));
					wp_enqueue_script( 'ita_formfilter');
				}else{
		...
		if(sizeof($contents) == 0):?>
			<tr class="no-items">
				<td colspan="<?php echo count($titles)-1;?>"><?php _e('No se ha encontrado información','ita');?></td>
			</tr>
		<?php 
		else:
...
function ita_class_form(
		...
		switch($type){
			...
			case "update":	$submit="Editar item";	break;
			default:
				ita_bad_msg("Error interno al identificar el Tipo de Formulario");
				wp_die();
		}
		echo '<h3>'.$title.'</h3>';
...
* @author Cristian Marin
*/
function ita_add_new_class_row( $tbl_name=null, $postvs=null, $db_fields=null, $name="item" ){
	global $wpdb;
	if(check_form_values($postvs, $db_fields)){
		...
		if ( $wpdb->insert( $tbl_name, $insertArray ) ){
			ita_good_msg("El nuevo ".$name." ha sido guardado.");
		}else{
			ita_bad_msg("Hubo un error al agregar el nuevo ".$name."; intenta nuevamente. :)");
		}
	
	}else{
		ita_bad_msg("No has ingresado los datos correctos para un nuevo ".$name."; intenta nuevamente. :)");
	}
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------
/**
...
* @author Cristian Marin
*/
function ita_edit_class_row( $tbl_name=null, $postvs=null, $db_fields=null, $name="item" ){
	global $wpdb;
...
*/			}
		...
		if( $result === false ){
			ita_bad_msg("Hubo un error al editar el ".$name."; intenta nuevamente. :)");
		}elseif ( $result == 0){
			ita_good_msg("Los valores son iguales. ".$name." no modificado.");
		}else{
			ita_good_msg($name." editado exitosamente.");
		}
	}else{
		var_dump($postvs);
		ita_bad_msg("No has ingresado los datos correctos para editar el ".$name."; intenta nuevamente. :)");
	}
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------
/**
...
* @author Cristian Marin
*/
function ita_bulk_delete( $tbl_name,$postvs,$name ){
	global $wpdb;
	if( isset( $postvs["delete"] ) ):
		...
		if( preg_match( "/^([0-9, ])*$/", $items ) ):
			...
			ita_good_msg($count.' '.$name.' han sido eliminados.');
		else:
			ita_bad_msg("No se pudo borrar los ".$name.", por favor pruebe nuevamente.");
		endif;
	else:
		ita_bad_msg("¡¿Estás mal de la cabeza?! Nada seleccionado!. Vuelve y selecciona elementos antes de hacer click nuevamente!");
	endif;
}
...
function ita_get_all($tables=null){
		global $wpdb;
/*	foreach($tables as $key => $table):
		$tables[$key]['tcode']='t'.str_pad($key, 3, "0", STR_PAD_LEFT); #000
	endforeach;
*/
	$SQLL=ita_get_all_select($tables,sizeof($tables)-1);
	$SQLL.=" FROM ".ita_get_all_from($tables,sizeof($tables)-1);
	if(($SQL_WHERE=ita_get_all_where($tables,sizeof($tables)-1))!=null){
		$SQLL.=" WHERE ".implode(" AND ",$SQL_WHERE);
	}
	$SQLL.=" ORDER BY ".ita_get_all_order($tables,sizeof($tables)-1);
	if(($SQL_GROUP=ita_get_all_group($tables,sizeof($tables)-1))!=null){
...
* @author Cristian Marin
*/
function ita_get_all_select($tables=null,$num=0){
	$sel_str=array();
	...
	if($num<=0){
		...
		return ita_get_all_select($tables,$num-1).",".$output;
	}
}
/**
* Add from clauses
...
* @author Cristian Marin
*/
function ita_get_all_from($tables=null,$num=0){
	$sel_str=array();
	foreach($tables[$num]['fields'] as $key => $field){
	...
	if($num<=0){
		...
		$output ="(";
			$output.=ita_get_all_from($tables,$num-1);
...
* @author Cristian Marin
*/
function ita_get_all_where($tables=null,$num=0){
	$whe_str=array();
	foreach($tables[$num]['fields'] as $key => $field){	
	...
	if($num<=0){
		...
		if(is_array($next=ita_get_all_where($tables,$num-1))):
...
* @author Cristian Marin
*/
function ita_get_all_order($tables=null,$num=0){
	$sel_str=array();
	foreach($tables[$num]['fields'] as $key => $field){
	...
	if($num<=0){
		...
		return $output.",".ita_get_all_order($tables,$num-1);
	}
}
/**
...
* @author Cristian Marin
*/
function ita_get_all_group($tables=null,$num=0){
	$grp_str=array();
	foreach($tables[$num]['fields'] as $key => $field){	
	...
	if($num<=0){
		...
		if(is_array($next=ita_get_all_order($tables,$num-1))):
			foreach($next as $entradas):
...
* @author Cristian Marin
*/
function ita_post_to_get($variable=null){
...
*/
function ita_plugin_style_loader_tag_function($tag){
...
add_filter('style_loader_tag', 'ita_plugin_style_loader_tag_function');
//---------------------------------------------------------------------------------------------------------------------------------------------------------
/*
